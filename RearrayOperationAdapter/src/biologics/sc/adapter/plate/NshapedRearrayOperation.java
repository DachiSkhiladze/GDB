...
/**
 * Distributes a list of isolates onto new plates. The fill order is N-shaped,
 * meaning a column is fully filled first before jumping to the next column:
 * A1, B1, C1, ..., A2, B2, C2, ... A3 ...
 *
 * @author tfiedler
 */
public class NshapedRearrayOperation implements RearrayOperationAdapter {
	...
	private List<Plate> fillPlates (PlateFactory plateFactory, List<Long> ids) {
		...
		while (! ids.isEmpty()) {
			...
			// search next writable well - n-shaped
			OuterWhile: while (true) {
				if (row > destRows) {
					row = 1;
					col ++;
				}
				if (null == dest || col > destCols) {
					row = col = 1;
					dest = plateFactory.createPlate();
					answer.add (dest);
				}
				for (; row <= destRows; row ++) {
