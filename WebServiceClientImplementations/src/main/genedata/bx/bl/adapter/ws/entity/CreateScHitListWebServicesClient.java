 ...
 * Hit Lists for a Screening Campaign.
 * 
 * It demonstrates the usage of 'multipart/form-data' input which is required
 * when more more than one data file needs to be transmitted with one web
 * service call.
 * 
 * In this case, the uploaded files - a "names" file and an "attributes" file -
 * must be either TSV or CSV formatted files.
 */
public class CreateScHitListWebServicesClient {

	private static final String postWsURL = "http://webserviceadmin:password@<tomcat>:8080/Biologics/ws/rest/hitlist/withNewIsolates";
	private static final String attributesFilename  = "CreateHLwithNewClones_attributes.txt";
	private static final String scHitListClonesFilename = "CreateHLwithNewClones_clone_names.txt";
	...
	public static void main(String[] args) throws Exception  {
		...
		URL attributes = CreateScHitListWebServicesClient.class.getResource(attributesFilename );
		URL scHitListClones = CreateScHitListWebServicesClient.class.getResource(scHitListClonesFilename);
		URL postUrl = new URL(postWsURL);
		
		File attributesFile = new File(attributes.toURI());
		File scHitListClonesFile = new File(scHitListClones.toURI());
		createScHitList(attributesFile, scHitListClonesFile, postUrl);
		
	}
	
	/**
	 * This method creates a Screen Campaign Hit List using a file containing 
	 * Antibody Clone attributes and a second file containing a list of
	 * Antibody Clones.
	 */
	private static void createScHitList(File attributesFile, File scHitListClonesFile, URL postUrl) throws IOException {
		...
		try {
			...
			addFilePart("names", scHitListClonesFile, outputStream);
